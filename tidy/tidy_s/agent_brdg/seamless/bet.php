<?php
    function agent_bet($jsondata)
    {
        ...
        // 插入log計算時間
        // if($_KERNEL_LOG['kernel_debug'] == TRUE)  outputlog("agent_".basename(__FILE__, '.php')."_start jsondata: ".$jsondata, "../../../../kernel/".$_KERNEL_LOG['dir'].getlogdate().$_KERNEL_LOG['kernel_log']);   
        ...
        // 轉換agent
        $changeagent = changeagent($jsondata, "bet");
        if(!empty($changeagent))
        {
            ...
            // 與平台確認bet
            $respond_data = send_get($_AGENTBRDG_URL['bet_url']."?jsondata=".$json_data);
        }

        // 插入log計算時間
        // if($_KERNEL_LOG['kernel_debug'] == TRUE)  outputlog("agent_".basename(__FILE__, '.php')."_end jsondata: ".$jsondata, "../../../../kernel/".$_KERNEL_LOG['dir'].getlogdate().$_KERNEL_LOG['kernel_log']);
